...
`Product` is a catalogue entry (category + brand + model, generates its own SKU). `ProductUnit` is one physical, serialized item — the thing that actually moves through statuses: `available → reserved/issued/repair/sold`, and back. `sold` and `removed` are the two terminal statuses — a unit only leaves `sold` via its own workflow (Client Return); a unit only leaves `removed` never (it's permanent: damaged beyond repair, lost, stolen, or written off). Every workflow has a `*Record` (the transaction header: `ReceivingRecord`, `DeliveryRecord`, `ReservationRecord`, `IssueRecord`, `RepairRecord`, `ClientReturnRecord`, `RemovalRecord`) and a `*Item` (the line linking that record to specific `ProductUnit`s). Every record type auto-generates a sequential human-readable number per year (`RCV-2026-0001`, `DLV-2026-0001`, `RMV-2026-0001`, etc.) in its own `save()`. `Product.total_units` (`ProductUnit.IN_STOCK_STATUSES`) is available + reserved + issued + repair only — `sold` and `removed` units have permanently left inventory and never count toward it.
...
3. **The six original workflow records follow one shared shape.** Create → (optional) correct header → cancel/release/return/resolve. Each has near-identical validation logic (only untouched units in the expected state can be released/returned/resolved — see `_reservation_item_can_be_released`, `_issue_item_can_be_returned`, `_repair_item_can_be_resolved`, `_delivery_unit_can_be_corrected` in `services.py`). This repetition is deliberate parallelism, not an accident — when fixing a bug in one workflow's guard logic, check whether the same bug exists in the other five. `RemovalRecord` is the deliberate exception: it's create-only, no cancel/release/return/resolve action, since the removal itself is permanent (same reasoning as `sold` having no direct "unsell" action — only Client Return reverses it).
...
7. **Workflow form field ordering.** On the six "Information" panels (Receive Stock/Add Unit, Create Delivery, Create Reservation, Create Issue, Create Repair, Create Client Return) and their matching Summary panels, fields follow one fixed sequence: (1) the record's date field first (Receiving/Entry Date, Delivery Date, Expected Release Date, Issue Date, Repair Date, Return Date), (2) then the external-party field(s) (Supplier, Client, Reserved For, Issued To, Received From), (3) then our own internal-reference fields (PO Number, Supplier Invoice Number, Receiver Name, Department, Branch/Site, Repair Location, Technician, Resolution, and similar), (4) Notes always last. Apply this to any new workflow page, and keep the form and its Summary panel in the same order (see the field-reorder rule below for why).
...
- New record type/model → add it to global search as part of that feature's implementation, not as an afterthought. Add a permission-gated group to `global_search()` in `apps/stock/selectors.py` (search fields + title/subtitle/href mapper) and confirm the frontend `GlobalSearch` component in `AppRouter.jsx` renders it via the existing generic group-rendering — it needs no per-type frontend change, only the backend group.
...
- After creating a new migration, run `python manage.py migrate` yourself to apply it against the dev database before considering the task done — don't leave migrations pending for manual application. This is separate from the no-test-running policy: migrations apply automatically as part of finishing the change; tests still only run when explicitly asked.
- When reordering fields on a form, also check and update any summary/preview panel showing the same fields, so both stay in sync — don't treat them as separate tasks unless told otherwise.
